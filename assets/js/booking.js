...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.selectRoom = (roomId) => {
        // Optimization: If already selected, just go to next step
    ...
    function clearErrors() {
        ...
        if (elements.guestError) elements.guestError.style.display = 'none';
        elements.checkInInput.classList.remove('input-error');
    }

    function showGuestError(msg) {
        if (elements.guestError) {
            elements.guestError.textContent = msg;
            elements.guestError.style.display = 'block';
        } else {
            alert(msg);
        }
    }
